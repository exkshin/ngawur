...
interface DropdownSupplierModel {
  value: SupplierModel | null,
  onChange?(value: SupplierModel): void,
  onOpenDropdown?(value: boolean): void,
  className?: HTMLProps<HTMLElement>["className"],
  showAllSupplierOption?: boolean,
  isError?: boolean,
  errorText?: string,
  useTitle?:boolean
  title?: string,
}
export const DropdownSupplier = (pParameter: DropdownSupplierModel) => {
      ...
      await AXIOS
        ...
        .then((data) => {
          if (data.success == true) {
            ...
            setXListSupplier(responseData);
            setXListSupplierTemp(responseData);
          } else {
  ...
  return <>
        ...
        <Sheet.Content>
          ...
          <Sheet.Scroller  >
            <div className='px-4 ' style={{ maxHeight: "70vh" }}>
              {!xIsLoading&& pParameter.showAllSupplierOption==true ? 
              <div
                className={`pb-2.5 border-b border-Grey text-neutral-100  text-14px pt-2.5`}
                onClick={(e) => {
                  if (pParameter.onChange != undefined && pParameter.onChange != null) {
                    pParameter.onChange(xAllSupplier);
                  }
              ...
              {xIsLoading ? <div>
                ...
                return (
                  <div
                    key={pIndex.toString()}
                    className={`pb-2.5 border-b border-Grey text-neutral-100  text-14px ${pIndex == 0&&pParameter.showAllSupplierOption!=true ? "" : "pt-2.5"
                      } `}
                    onClick={(e) => {
    ...
    <div className={`w-full ${pParameter.className}`}>
      ...
      <div className="relative w-full mt-2.5" onClick={() => {
        ...
        <input
          placeholder={""}
          value={pParameter.value == null ? "" : pParameter.value.namasupplier ?? ""}
          type={"text"}
          readOnly={true}
