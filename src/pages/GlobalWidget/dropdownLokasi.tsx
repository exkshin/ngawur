...
interface DropdownLokasiModel {
  value: LokasiModel | null,
  onChange?(value: LokasiModel): void,
...
export const DropdownLokasi = (pParameter: DropdownLokasiModel) => {
  ...
  const [xListLokasiTemp, setXListLokasiTemp] = useState([] as LokasiModel[]);
  ...
  async function getListLokasi() {
    ...
    try {
      ...
      await AXIOS
        ...
        .then((res) => res.data)
        .then((data) => {
          let responseData = data.rows as LokasiModel[];
          if (pParameter.isSupplier == true) {
            responseData = data.data as LokasiModel[];
          }
          setXListLokasi(responseData);
          setXListLokasiTemp(responseData);
        }
        );
    } catch (error) {
      console.log(error);
  ...
  function searchLokasi(pSearch: string) {
    let listLokasi = [] as LokasiModel[];
    xListLokasi.forEach((pLokasi) => {
      if ((pLokasi.nama ?? "").toUpperCase().includes(pSearch.toUpperCase())) {
        listLokasi.push(pLokasi);
      }
    })
    setXListLokasiTemp(listLokasi);
  }
              ...
              <div className="relative w-full">
                <input
                  onChange={(e) => {
                    ...
                    searchLokasi(e.target.value);
                  }}
                  placeholder={"Cari Lokasi"}
                  value={xSearch}
                  className={`${defaultInputCSS((xSearch !== "" && xSearch !== undefined), false, false)}`}
                  onSubmit={(value) => searchLokasi(xSearch)}
                  inputMode="search"
                  type="text"
                  enterKeyHint="search"
                  onKeyUp={(e) => {
                    if (e.key == "Enter") {
                      searchLokasi(xSearch);
                    }
                  }}
                />
              </div>
          ...
          <Sheet.Scroller>
            <div className='px-4' style={{ maxHeight: "70vh" }}>
              {xIsLoading ? <div>
                ...
                xListLokasiTemp.map((pLokasi, pIndex) => {
                  return <div
                    key={pIndex.toString()}
