...
interface BottomSheetDetailHutangModel {
  hutang: DataHutang | null,
  open: boolean,
  onClose(value: boolean): void,
}

export const BottomSheetDetailHutang = (pParameter: BottomSheetDetailHutangModel) => {
  const [xTotal, setXTotal] = useState(0.0);

  useEffect(() => {
    if (pParameter.hutang != null && pParameter.hutang != undefined && pParameter.hutang.daftarbarang != undefined) {
      let total = 0.0;
      pParameter.hutang.daftarbarang.forEach(pBarang => {
        total += (parseFloat(pBarang.hargabeli ?? "0.0") * parseFloat(pBarang.jml ?? "0.0"))
      });
      setXTotal(total);
    }
  }, [pParameter.hutang])
        ...
        <Sheet.Content>
          ...
          <Sheet.Scroller>
            <div className='px-4' style={{ maxHeight: "70vh" }}>
              {pParameter.hutang != undefined && pParameter.hutang != null && pParameter.hutang?.daftarbarang?.map((pBarang, pIndex) => {
                return <div
                  key={pIndex.toString()}
              ...
              `}
                >
                  ...
                  <p>{numberSeparatorFromString((pBarang.jml ?? "0").split(".")[0])} X {numberSeparatorFromString((pBarang.hargabeli ?? "0").split(".")[0])}</p>
                    {numberSeparatorFromString((parseFloat(pBarang.hargabeli ?? "0.0") * parseFloat(pBarang.jml ?? "0.0")).toString().split(".")[0])}
                  </div>
                </div>;
              })}
              <div
